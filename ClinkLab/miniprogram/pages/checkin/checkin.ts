...
Page({
  ...
  async confirmCheckin(e: WechatMiniprogram.TouchEvent): Promise<void> {
    ...
    const method = (e.currentTarget.dataset.method as string) === 'PHONE_LAST4'
      ...
      : 'QR' as CheckinMethod;
    // 双人票：按扫码定位到的座位（1 或 2）核销；手机号 / 单人为座位 1
    const seat = method === 'QR' && this.data.resolved ? (this.data.resolved.seat_no || 1) : 1;
    this.setData({ busy: true, error: '' });
    try {
      const res = await performCheckin(id, method, undefined, seat);
      this.setData({ lastResult: res, busy: false, resolved: null, search: null, last4: '' });
      if (!res.already) toast(res.message || '签到成功', 'success');
    } catch (e2) {
