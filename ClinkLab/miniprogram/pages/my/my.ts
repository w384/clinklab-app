...
import { listMyRegistrations, cancelRegistration, Registration, canReview } from '../../services/registration';
...
function badgeCls(r: Registration): string {
  if (r.option_type === 'WAITLIST') return 'waitlist'; // 候补（含自动转候补）
...
Page({
  ...
  goReview(e: WechatMiniprogram.TouchEvent): void {
    const id = e.currentTarget.dataset.id as number;
    wx.navigateTo({ url: `/pages/review/review?registrationId=${id}` });
  },
  // 待支付报名（含候补补位转正后）→ 结果页继续支付
  goPay(e: WechatMiniprogram.TouchEvent): void {
    const id = e.currentTarget.dataset.id as number;
    wx.navigateTo({ url: `/pages/pay-result/pay-result?id=${id}` });
  },
