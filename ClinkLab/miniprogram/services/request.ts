...
export function request<T = unknown>(options: RequestOptions): Promise<T> {
  const first = doRequest<T>(options);
  // GET 请求在"网络/超时"（code -1）时自动重试一次：GET 幂等安全；POST 不重试（避免重复下单/重复支付）
  if ((options.method || 'GET') !== 'GET') return first;
  return first.catch((e: ApiError) => {
    if (e && e.code === -1) return doRequest<T>(options);
    throw e;
  });
}

function doRequest<T = unknown>(options: RequestOptions): Promise<T> {
...
export const del = <T = unknown>(url: string, data?: Record<string, any>): Promise<T> => request<T>({ url, method: 'DELETE', data });
